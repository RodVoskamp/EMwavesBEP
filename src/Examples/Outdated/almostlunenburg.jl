...
function κ2(p)
    ω^2*(2 - (BEAST.norm(p)^2)) #lunenburch
    #ω^2 #PMCHWT
end
...
#isplay(heatmap(clamp.(real.(norm.(SLj+DLm)), 0.0, 2.0)))

vals = BEAST.grideval(pts, uj, X)
vals2 = BEAST.grideval(pts, um, Y)
display(plot(surface(t,t2,getindex.(real(vals+vals2),1),camera=(0,90))))
...
Yhemi = BEAST.restrict(X, hemi)
tYhemi = BEAST.ttrace(Yhemi, bnd_hemi)
fcr, geo = facecurrents(uj, tYhemi)

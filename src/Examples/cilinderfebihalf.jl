...
function ω2ϵ(p)
    ...
    ϵ = (1/(1-cutoff))
    if r > cutoff
        ω^2*Diagonal([1,1,1])*(Rot(p)*(Diagonal([ϵ^2,0,0]))*Roti(p))
...
function μ1(p)
    ...
    r = sqrt(p[3]^2+p[2]^2)
    if r > cutoff
        (Rot(p)*(Diagonal([0,r^2/(r-cutoff),1]))*Roti(p))*Diagonal([1,1,1])
...
gif(anim, "half_o7r325o10_c.gif", fps = 15)
...
gif(anim, "half_o7r325o10_b.gif", fps = 15)
...
gif(anim, "half_o7r325o10_a.gif", fps = 15)
